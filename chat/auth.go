...
func (h *authHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if _, err := r.Cookie("auth"); err == http.ErrNoCookie {
		...
		// authHandler typeの h のnextフィールドに格納されたobjectが持つServeHTTPメソッドをcallする
...
func loginHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "callback":
		...
		// r.URLには今のGETリクエストの情報が入っている
		// 更にRawQueryを実行することでクエリパラメータを取得できる
		//
		// creds にはアクセストークンやそのスコープが入っている
